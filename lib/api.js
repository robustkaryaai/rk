...
export const userAPI = {
    ...
    async syncUserToAppwrite(user, avatarUrl = '', providerAccessToken = null, providerRefreshToken = null) {
        try {
            ...
            // First check by ID (most reliable for Appwrite Auth)
            try {
                ...
                // Store Google tokens if provided (optional - won't error if field doesn't exist in schema)
                if (providerAccessToken) {
                    try {
                        updateData.googleAccessToken = providerAccessToken;
                    } catch (e) {
                        // Field might not exist in schema, that's okay
                    }
                }
                if (providerRefreshToken) {
                    try {
                        updateData.googleRefreshToken = providerRefreshToken;
                    } catch (e) {
                        // Field might not exist in schema, that's okay
                    }
                }
                ...
                console.log('[User Sync] Updated user in Appwrite DB:', userId);
                return true;

            } catch (err) {
                // Document not found by ID (expected for new users)
                const userData = {
                    name: name,
                    email: email,
                    avatar: avatarUrl || ''
                ...
                if (providerAccessToken) {
                    try {
                        userData.googleAccessToken = providerAccessToken;
                    } catch (e) {
                        // Field might not exist in schema, that's okay
                    }
                }
                if (providerRefreshToken) {
                    try {
                        userData.googleRefreshToken = providerRefreshToken;
                    } catch (e) {
                        // Field might not exist in schema, that's okay
                    }
                }
            ...
            console.error('[User Sync] Error syncing user to Appwrite:', error);
            return false;
        }
    }
};
