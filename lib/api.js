...
// Backend API URL
export const API_BASE_URL = 'https://rk-ai-backend.onrender.com';
...
export const deviceAPI = {
    ...
    // Queue hardware command (Mute, Volume, Yes, No)
    async queueCommand(slug, commandType, payload = {}) {
        try {
            console.log(`[Queue Command] Queueing ${commandType} for device: ${slug}`);
            const response = await fetch(`${API_BASE_URL}/device/${slug}/command`, {
                method: 'POST',
            ...
            if (!response.ok) {
                const text = await response.text();
                console.error(`[Queue Command] Failed: ${text}`);
                return false;
            }
            return true;
        } catch (error) {
